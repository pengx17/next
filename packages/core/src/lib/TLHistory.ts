...
import type { TLAppStateModel, TLDocumentModel, TLEventMap } from '~types'
...
export class TLHistory<S extends TLShape, K extends TLEventMap> {
  ...
  deserialize = (document: TLDocumentModel, state: TLAppStateModel) => {
    // const { pages } = document
    // const { currentPageId, selectedIds } = state
    ...
    //         } else {
    //           // Create the shape
    //           const ShapeClass = this.app.getShapeConstructor(serializedShape.type)
